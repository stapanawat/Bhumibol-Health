...
const submit = () => {
    form.post(route('admin.sitemap-categories.store'));
};
</script>
    ...
    <AppSidebarLayout>
        <div class="max-w-xl mx-auto space-y-6">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-2xl font-bold tracking-tight">Create Category</h2>
                    <p class="text-muted-foreground">Add a new category for sitemap links.</p>
                </div>
            </div>
            ...
            <form @submit.prevent="submit" class="space-y-8">
                <div class="grid gap-6 p-6 border rounded-lg bg-card">
                    <div class="space-y-2">
                        ...
                        <Input id="name_th" v-model="form.name_th" required placeholder="e.g. บริการของเรา" />
                        <div v-if="form.errors.name_th" class="text-destructive text-sm">{{ form.errors.name_th }}</div>
                    </div>

                    <div class="space-y-2">
                        <Label for="name_en">Name (English)</Label>
                        <Input id="name_en" v-model="form.name_en" placeholder="e.g. Our Services" />
