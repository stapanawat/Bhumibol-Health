...
const searchTerm = ref(''); // We need to capture search term from CommandInput if possible, but Shadcn CommandInput doesn't easy expose v-model. 
// Workaround: We will rely on the fact that if it's empty, user typed something not in list.
// Actually, `CommandInput` v-model works.

const submit = () => {
    form.post(route('admin.sitemaps.store'));
};

const selectNewCategory = () => {
    if (!searchTerm.value) return;
    form.sitemap_category_id = searchTerm.value; // Store the NAME string
    ...
    <AppSidebarLayout>
        <div class="max-w-xl mx-auto space-y-6">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-2xl font-bold tracking-tight">Create Link</h2>
                    <p class="text-muted-foreground">Add a new link to the sitemap.</p>
                </div>
            </div>
            ...
            <form @submit.prevent="submit" class="space-y-8">
                <div class="grid gap-6 p-6 border rounded-lg bg-card">
                    ...
                    <div class="space-y-2">
                        ...
                        <Input id="title_th" v-model="form.title_th" required placeholder="e.g. หน้าแรก" />
                        <div v-if="form.errors.title_th" class="text-destructive text-sm">{{ form.errors.title_th }}</div>
                    </div>

                    <div class="space-y-2">
                        <Label for="title_en">Title (English)</Label>
                        <Input id="title_en" v-model="form.title_en" placeholder="e.g. Home" />
                        <div v-if="form.errors.title_en" class="text-destructive text-sm">{{ form.errors.title_en }}</div>
                    </div>

                    <div class="space-y-2">
                        <Label for="link">URL / Link</Label>
                        <Input id="link" v-model="form.link" required placeholder="https://..." />
